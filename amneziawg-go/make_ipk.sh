...
tar "--exclude=./control.tar" "--exclude=./control" "--exclude=./$OUR_BASENAME" "--exclude=./$IPK_BASENAME" "--exclude=./make_ipk*.log" -cvf data.tar $INPUT_FILES
...
# create PACKAGE.tar
#
tar --numeric-owner --group=0 --owner=0 -cf "packagetemp.tar" "./control.tar.gz" "./data.tar.gz" "./debian-binary"
